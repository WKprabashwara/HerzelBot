...
import pymongo
from Herzel import app,DB_URI
from Herzel.utils.filter_groups import nm_g
from lang import get_command
from Herzel.utils.commands import command
from Herzel.utils.lang import language
from Herzel.utils.custom_filters import can_change_filter
...
async def customize_night(client, message: Message, _):
    herzel = await message.reply(_["nm2"])
    if message.chat.type == "private":
        return await herzel.edit(_["nm5"])
    if message.chat.type == "channel":
        ...
        if len(message.command) < 3:
            return await herzel.edit(_["nm11"])
        if "|" in parameter:
            zone, ctime, otime = parameter.split("|")
        else:
            return await herzel.edit(_["nm12"])
        zone = zone.strip()
        ctime = ctime.strip()
        otime = otime.strip()
        if len(ctime) != 11:
            return await herzel.edit(_["nm13"])
        if len(otime) != 11:
            return await herzel.edit(_["nm13"])
        if not zone and ctime and otime:
            return await herzel.edit(_["nm14"])
        ttime = dateparser.parse("now", settings={"TIMEZONE": f"{zone}", "DATE_ORDER": "YMD"})
        if ttime is None or otime is None or ctime is None:
            return await herzel.edit("Please enter valid `date`, `time` & `zone`")
        ...
        if cctime == ootime:
            return await herzel.edit(_["nm15"])
        if not ootime > cctime and not cctime < ootime:
            return await herzel.edit(_["nm16"])
        if cctime > ootime:
            return await herzel.edit("Chat closing time can't be greater than opening time")
        chats = nightmod.find({})
        for c in chats:
            if message.chat.id == c["id"] and c["valid"] is True:
                ...
                await herzel.edit("**Nightmode already set**\n\n__I am updating the zone, closing time and opening time with the new zone, closing time and opening time__")
                await asyncio.sleep(2)
                return await herzel.edit(f"**Nightmode Updated Successfully in {message.chat.title} chat**")
        nightmod.insert_one({"id": message.chat.id,"valid": True,"zone": zone,"ctime": cctime,"otime": ootime})
        await herzel.edit(f"**Nightmode set successfully in {message.chat.title} chat !**")

...
from Herzel import app,BOT_ID
from Herzel.utils.filter_groups import cbot
from lang import get_command
from Herzel.utils.lang import language
from Herzel.mongo import chatb
from Herzel.plugins.antlangs import get_arg
from Herzel.utils.custom_filters import admin_filter
...
async def szcbot(_, message: Message):
    ...
    if chat:
        ...
        try:
           ...
           affiliateplus = requests.get(f"https://api.affiliateplus.xyz/api/chatbot?message={text}&botname=Herzel&ownername=@supunma&user={chat_id}")
           textmsg = (affiliateplus.json()["message"])
           if "Affiliate+" in textmsg:
               textmsg = textmsg.replace("Affiliate+", "Herzel")
           ...
           trtoen = (message.text if lang=="en" else tr.translate(message.text, dest="en").text).replace(" ", "%20")
           text = trtoen.replace(" ", "%20") if len(message.text) < 2 else trtoen
           safeone = requests.get(f"https://api.safone.tech/chatbot?message={text}&bot_name=Herzel&bot_master=Supun&user_id={user_id}")
...
AI based chatbot allows herzel to talk and provides a more interactive group chat experience.
...
**Language Support**
Herzel AI chatbot support almost all languages in world .
